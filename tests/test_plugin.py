...
@pytest.mark.parametrize('splinter_session_scoped_browser', [False])
def test_clean_cookies(
        request, monkeypatch, httpserver, cookie_name, splinter_webdriver, splinter_session_scoped_browser):
    ...
    monkeypatch.setattr('pytest_splinter.plugin.unlink', mocked_unlink)
